            ...
            onClick = {
                ...
                val header = DocumentHeader(
                    ...
                    mapOf(
                        ...
                        DocumentType.ORDER to (AppPreferences.backgroundImageUri(context, DocumentType.ORDER)?.let { runCatching { context.contentResolver.openInputStream(Uri.parse(it)).use(BitmapFactory::decodeStream) }.getOrNull() } ?: BitmapFactory.decodeResource(context.resources, R.drawable.ic_launcher_logo_bitmap)),
                        DocumentType.REQUEST to (AppPreferences.backgroundImageUri(context, DocumentType.REQUEST)?.let { runCatching { context.contentResolver.openInputStream(Uri.parse(it)).use(BitmapFactory::decodeStream) }.getOrNull() } ?: BitmapFactory.decodeResource(context.resources, R.drawable.ic_launcher_logo_bitmap)),
                        DocumentType.RECEIPT to (AppPreferences.backgroundImageUri(context, DocumentType.RECEIPT)?.let { runCatching { context.contentResolver.openInputStream(Uri.parse(it)).use(BitmapFactory::decodeStream) }.getOrNull() } ?: BitmapFactory.decodeResource(context.resources, R.drawable.ic_launcher_logo_bitmap))
                    ), mapOf(
