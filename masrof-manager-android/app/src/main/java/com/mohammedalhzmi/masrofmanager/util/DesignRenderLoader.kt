...
object DesignRenderLoader {
    fun design(context: Context, type: DocumentType): DocumentDesignEntity? = runBlocking {
        val db = Room.databaseBuilder(context.applicationContext, MasrofDatabase::class.java, "masrof-db")
            .addMigrations(MasrofDatabase.MIGRATION_3_4, MasrofDatabase.MIGRATION_4_5, MasrofDatabase.MIGRATION_5_6, MasrofDatabase.MIGRATION_6_7, MasrofDatabase.MIGRATION_7_8, MasrofDatabase.MIGRATION_8_9).build()
    ...
    fun elements(context: Context, type: DocumentType): List<DesignElementEntity> = runBlocking {
        val db = Room.databaseBuilder(context.applicationContext, MasrofDatabase::class.java, "masrof-db")
            .addMigrations(MasrofDatabase.MIGRATION_3_4, MasrofDatabase.MIGRATION_4_5, MasrofDatabase.MIGRATION_5_6, MasrofDatabase.MIGRATION_6_7, MasrofDatabase.MIGRATION_7_8, MasrofDatabase.MIGRATION_8_9)
            .build()
        try {
