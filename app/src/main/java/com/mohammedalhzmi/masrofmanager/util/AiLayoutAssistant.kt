...
object AiLayoutAssistant {
    ...
    suspend fun plan(apiKey: String, endpoint: String, instruction: String, elements: List<DesignElementEntity>, design: DocumentDesignEntity?): List<Command> {
        ...
        val system = "أنت مساعد تنسيق مستندات مالية عربية. أعد JSON فقط على شكل مصفوفة أوامر. الأفعال المسموحة: MOVE, RESIZE, ROTATE, UPDATE_TEXT, STYLE_TEXT, ADD_TEXT, ADD_SHAPE, ADD_QR, DELETE, PAGE. لا تنفذ أي كود ولا تغير الصلاحيات. الإحداثيات بنقاط PDF. لكل أمر استخدم مفاتيح مناسبة: action,targetId,type,content,x,y,width,height,rotation,fontSize,textColor,textAlign,pageWidth,pageHeight,orientation,margin,backgroundColor. إذا كان الطلب غامضًا أعد مصفوفة فارغة."
